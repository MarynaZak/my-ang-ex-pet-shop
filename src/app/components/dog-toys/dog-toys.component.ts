...
@Component({
  selector: 'app-dog-toys',
  templateUrl: './dog-toys.component.html',
  styleUrl: './dog-toys.component.css'
})
export class DogToysComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.searching.text$.subscribe(() => {
      this.loadData.getData().subscribe((data: any) => {this.tiles = this.searching.filter(data['dog toys'])})
    })
  }
