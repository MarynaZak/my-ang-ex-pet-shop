...
@Component({
  selector: 'app-dog-bowls',
  templateUrl: './dog-bowls.component.html',
  styleUrl: './dog-bowls.component.css'
})
export class DogBowlsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.searching.text$.subscribe(() => {
      this.loadData.getData().subscribe((data: any) => {this.tiles = this.searching.filter(data['dog bowls'])})
    })
  }
